...
func (_q *UserQuery) QueryPassword() *PasswordQuery {
...
func (_q *UserQuery) QueryUserRole() *UserRoleQuery {
...
func (_q *UserQuery) WithPassword(opts ...func(*PasswordQuery)) *UserQuery {
	query := (&PasswordClient{config: _q.config}).Query()
...
func (_q *UserQuery) WithUserRole(opts ...func(*UserRoleQuery)) *UserQuery {
	query := (&UserRoleClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) loadPassword(ctx context.Context, query *PasswordQuery, nodes []*User, init func(*User), assign func(*User, *Password)) error {
	...
	query.Where(predicate.Password(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.PasswordColumn), fks...))
	}))
...
func (_q *UserQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*User, init func(*User), assign func(*User, *Role)) error {
	...
	nids := make(map[uint]map[*User]struct{})
...
func (_q *UserQuery) loadUserRole(ctx context.Context, query *UserRoleQuery, nodes []*User, init func(*User), assign func(*User, *UserRole)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
