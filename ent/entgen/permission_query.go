...
func (_q *PermissionQuery) QueryRolePermission() *RolePermissionQuery {
...
func (_q *PermissionQuery) FirstID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (_q *PermissionQuery) FirstIDX(ctx context.Context) uint {
...
func (_q *PermissionQuery) OnlyID(ctx context.Context) (id uint, err error) {
	var ids []uint
...
func (_q *PermissionQuery) OnlyIDX(ctx context.Context) uint {
...
func (_q *PermissionQuery) IDs(ctx context.Context) (ids []uint, err error) {
...
func (_q *PermissionQuery) IDsX(ctx context.Context) []uint {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *PermissionQuery) WithRolePermission(opts ...func(*RolePermissionQuery)) *PermissionQuery {
	query := (&RolePermissionClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *PermissionQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Permission, error) {
	...
	if query := _q.withRolePermission; query != nil {
		if err := _q.loadRolePermission(ctx, query, nodes,
			func(n *Permission) { n.Edges.RolePermission = []*RolePermission{} },
			func(n *Permission, e *RolePermission) { n.Edges.RolePermission = append(n.Edges.RolePermission, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *PermissionQuery) loadRoles(ctx context.Context, query *RoleQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *Role)) error {
	...
	byID := make(map[uint]*Permission)
	nids := make(map[uint]map[*Permission]struct{})
...
func (_q *PermissionQuery) loadRolePermission(ctx context.Context, query *RolePermissionQuery, nodes []*Permission, init func(*Permission), assign func(*Permission, *RolePermission)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uint]*Permission)
	for i := range nodes {
	...
	query.Where(predicate.RolePermission(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(permission.RolePermissionColumn), fks...))
	}))
...
func (_q *PermissionQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(permission.Table, permission.Columns, sqlgraph.NewFieldSpec(permission.FieldID, field.TypeUint))
	_spec.From = _q.sql
