...
func (_c *PermissionCreate) SetDescription(v constants.Permission) *PermissionCreate {
...
func (_c *PermissionCreate) AddRoleIDs(ids ...uint) *PermissionCreate {
...
func (_c *PermissionCreate) AddRolePermission(v ...*RolePermission) *PermissionCreate {
	ids := make([]uint, len(v))
	for i := range v {
		ids[i] = v[i].ID
	}
...
func (_c *PermissionCreate) check() error {
	if _, ok := _c.mutation.CreatedAt(); !ok {
		return &ValidationError{Name: "created_at", err: errors.New(`entgen: missing required field "Permission.created_at"`)}
	}
	if _, ok := _c.mutation.UpdatedAt(); !ok {
		return &ValidationError{Name: "updated_at", err: errors.New(`entgen: missing required field "Permission.updated_at"`)}
	}
	if _, ok := _c.mutation.Description(); !ok {
		return &ValidationError{Name: "description", err: errors.New(`entgen: missing required field "Permission.description"`)}
	}
	...
	if v, ok := _c.mutation.ID(); ok {
		if err := permission.IDValidator(v); err != nil {
			return &ValidationError{Name: "id", err: fmt.Errorf(`entgen: validator failed for field "Permission.id": %w`, err)}
		}
	}
	return nil
}
...
func (_c *PermissionCreate) createSpec() (*Permission, *sqlgraph.CreateSpec) {
	...
	if nodes := _c.mutation.RolePermissionIDs(); len(nodes) > 0 {
		edge := &sqlgraph.EdgeSpec{
			Rel:     sqlgraph.O2M,
			Inverse: true,
